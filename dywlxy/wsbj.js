...
$(document).ready(function () {
	...
	if (currentUrl.indexOf("online-class") != -1) {
		$(".header").remove();
        if($(".clickSave").length != 0){
            $(".clickSave").click();
            setTimeout(function () {
                $(".detail-tab").find("img").eq(0).click();
		    }, 1500);
        }
		else {
            setTimeout(function () {
...
function init_compontent() {
    ...
    $("#End").bind("click",
      ...
      /*监听输入框的回车操作*/
    $('#iptTime').bind('keypress',function(event){
...
function init_alllist() {
    ...
    var requestUri = "/gwapi/dywlxynet/api/web/class/archiveMyCourse";
    var headerConfig = setHeader(baseInfo.userConfig);
    $.ajax({
        url: requestUri,
        type: 'post',
        ...
        success: function (dataSource) {
            ...
            if (typeof (dataSource) != "undefined") {
                ...
                for (var i = 0; i < dataSource.datalist.length; i++) {
                    var courseListRow = {};
                    courseListRow.classId =  dataSource.datalist[i].classId;
		            courseListRow.courseDuration =  dataSource.datalist[i].courseDuration;
		            courseListRow.courseHour =  dataSource.datalist[i].courseHours;
		            courseListRow.courseCode =  dataSource.datalist[i].courseCode;
		            courseListRow.courseName =  dataSource.datalist[i].courseName;
		            courseListRow.id =  dataSource.datalist[i].courseId;
		            courseListRow.examFlag =  dataSource.datalist[i].examFlag;
...
function updateEnd(currentPlayTime) {
    ...
    var requestUri = "/gwapi/dywlxynet/api/web/course/endCourseStudy";
    var headerConfig = setHeader(baseInfo.userConfig);
    $.ajax({
        url: requestUri,
        type: 'post',
        ...
        success: function (data) {
            if (data.success == false && data.message.indexOf("每天规则限制") != -1) {
                maFlag = true;
