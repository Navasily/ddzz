...
function loadScript(){
	$("body").append("<script src=" +  host + "/ddzz/dkt/loadGloableData.js'></script>");
...
function init_baseInfo() {
    ...
    baseInfo.publishMonth = new Array();
	//["202201","202203","202204","202205","202206","202207","202208","202209","202210","202211"];
	...
	$(".el-select-dropdown__item").each(function(){
		let tx = $(this).text();
		if(tx.indexOf("全部")!= -1) return;
		tx = tx.replace("年","").replace("月","");
		baseInfo.publishMonth.push(tx);
	});
...
function getTotalHours() {
    ...
    data.publishMonth = baseInfo.publishMonth[baseInfo.publishMonth.length-1];
    $.ajax({　　　　
        ...
        if (typeof(data) != "undefined") {
            baseInfo.totalTime = data.data.sum;
...
function taskProcess(currentCourse, num) {
    ...
    this.haveRead = function(that) {
        ...
        postData = {
            ...
            publishMonth: Number(that.currentCourse.publishMonth),
            courseIds: i
        };
    ...
    this.updateStudyRecordEnd = function(that) {
        ...
        var receive = {
            ...
            orgName: baseInfo.userInfo.orgName,
            playFlag: baseInfo.playFlag,
            publishMonth: that.currentCourse.publishMonth,
            userId: baseInfo.userInfo.userId,
            studyTimes: getStudyTimes
        };
        var signatureEntity = {
...
//finish
function init_alreadystudylist(currentMonth) {
	if(currentMonth >= baseInfo.publishMonth.length){
		init_alllist(0);
		return;
	}
    var postData = {
        userId: baseInfo.userInfo.userId,
        publishMonth: Number(baseInfo.publishMonth[currentMonth])
    };
    $.ajax({
        ...
        if (typeof(dataSource) != "undefined") {
            if (dataSource.data == null) {
				currentMonth++;
                init_alreadystudylist(currentMonth);
                return;
            }
            var temptotaltime = 0;
            for (var i = 0; i < dataSource.data.length; i++) {
				...
				tempCourse.publishMonth = baseInfo.publishMonth[currentMonth];
            ...
            setTimeout(function(){init_alreadystudylist(currentMonth);}, 3000);
        }
    },
    function error(e) {
         setTimeout(function(){init_alreadystudylist(currentMonth);}, 5000);
    });
}
//finishs
function init_alllist(currentMonth) {
	if(currentMonth >= baseInfo.publishMonth.length){
        init_studylist();
		init_publishMonth();
		return;
	}
	$("#lblresult").html("正在初始化 "+baseInfo.publishMonth[currentMonth] + " 课程1");
	$.ajax({
        url: 'http://dkt.dtdjzx.gov.cn/course/findByPublishMonth?publishMonth=' + baseInfo.publishMonth[currentMonth],
        type: 'post',
        contentType: 'application/json;charset=utf-8',
    }).then(function(dataSource) {
       if (typeof(dataSource) != "undefined") {
		   ...
		   for(let i=0;i<data.length;i++){
			   if(data[i].courseHours>0){
				   allCourseList.push(data[i]);		   
			   }
		   }
		   init_subCourse(currentMonth);
	   }
    },
    function error(e) {
        setTimeout(function(){init_alllist(currentMonth)}, 5000);
    });
	
}

function init_subCourse(currentMonth){
	$("#lblresult").html("正在初始化 "+baseInfo.publishMonth[currentMonth] + " 课程2");
	let postData = {
        publishMonth: Number(baseInfo.publishMonth[currentMonth])
    };
	
	$.ajax({
        url: 'http://dkt.dtdjzx.gov.cn/subcourse/subCoursePage?pagenum=0&pagesize=' + baseInfo.pageSize,
        type: 'post',
		data: JSON.stringify(postData),
        contentType: 'application/json;charset=utf-8',
    }).then(function(dataSource) {
       if (typeof(dataSource) != "undefined") {
		   ...
		   currentMonth++;
		   init_alllist(currentMonth);
	   }
    },
    function error(e) {
        setTimeout(function(){init_subCourse(currentMonth)}, 5000);
    });
}

//finish
function init_studylist() {
	let preCourseList = allCourseList;
...
function init_publishMonth() {
    var selectOptions = "";
    for (var i = 0; i < baseInfo.publishMonth.length; i++) {
        selectOptions += "<option value='" + baseInfo.publishMonth[i] + "'>" + baseInfo.publishMonth[i] + "</option>";
    }
    ...
    $("#asyncSelect").change(function() {
        ...
        for (var i = 0; i < currentAsyncCount; i++) {
            ...
            $("#classInfo").append("<div>" + lblText3 + currentPlayTime + lblText2 + lblCurrentCourseTitle  + "</div>");
        }
...
function init_courseSelect() {
    ...
    for (var i = 0; i < courseList.length; i++) {
        selectOptions += "<option value='" + i + "'>" + courseList[i].courseName + "（时长：" + courseList[i].courseDuration + "分钟|学时：" + courseList[i].courseHours + "|年度：" + courseList[i].publishMonth + "）</option>";
    }
